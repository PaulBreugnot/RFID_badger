...
void MFRC522::PCD_WriteRegister(PCD_Register reg, byte value) {
	...
	// ESP_LOGI("WRITE", "Write single byte to %.2x : %.2x", reg >> 1, value);
	ESP32CPP::GPIO::low((gpio_num_t)_chipSelectPin);
	m_spi.transfer(data, 2);
	ESP32CPP::GPIO::high((gpio_num_t)_chipSelectPin);
...
void MFRC522::PCD_WriteRegister(PCD_Register reg, byte count, byte* values) {
	//ESP_LOGI("REG", "Writting values to reg %.2x :", reg);
	// print_hex(values, count);
	...
	ESP32CPP::GPIO::high((gpio_num_t)_chipSelectPin);
	delete[] pData;
	//byte* read_values = new byte[count+1];
	//PCD_ReadRegister(reg, count, read_values, 0);
	//ESP_LOGI("REG", "Read values :");
	//print_hex(read_values, count);
} // End PCD_WriteRegister()
...
byte MFRC522::PCD_ReadRegister(PCD_Register reg) {
	...
	// ESP_LOGI("READ", "Data[0] (complete address transfered) : %.2x", data[0]);

	ESP32CPP::GPIO::low((gpio_num_t)_chipSelectPin);
	m_spi.transfer(data, 2);
	ESP32CPP::GPIO::high((gpio_num_t)_chipSelectPin);

	// ESP_LOGI("READ", "Data[0] response from reg %.2x : %.2x", reg >> 1, response[0]);
	// ESP_LOGI("READ", "Final 0 response from reg %.2x : %.2x (returned value)", reg >> 1, data[1]);
	return data[1];
...
void MFRC522::PCD_Init() {
	...
	///////////////////////////////////////////////
	ESP_LOGI("CHECK REG", "Checking registers");
	// Reset baud rates
	...
	// f_timer = 13.56 MHz / (2*TPreScaler+1) where TPreScaler = [TPrescaler_Hi:TPrescaler_Lo].
	// TPrescaler_Hi are the four low bits in TModeReg. TPrescaler_Lo is TPrescalerReg.
	PCD_ReadRegister(TModeReg);			// TAuto=1; timer starts automatically at the end of the transmission in all communication modes at all speeds
	PCD_ReadRegister(TReloadRegH);		// Reload timer with 0x3E8 = 1000, ie 25ms before timeout.
	PCD_ReadRegister(TPrescalerReg);  // TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25μs.
	PCD_ReadRegister(TReloadRegH);	// Reload timer with 0x3E8 = 1000, ie 25ms before timeout.
	PCD_ReadRegister(TReloadRegL);

	PCD_ReadRegister(TxASKReg);	   // Default 0x00. Force a 100 % ASK modulation independent of the ModGsPReg register setting
	PCD_ReadRegister(ModeReg);		// Default 0x3F. Set the preset value for the CRC coprocessor for the CalcCRC command to 0x6363 (ISO 14443-3 part 6.2.4)
	/////////////////////////////////////////////////////
...
void MFRC522::PCD_DumpVersionToSerial() {
	...
	ESP_LOGI(LOG_TAG, "%s", oss.str().c_str());
	// When 0x00 or 0xFF is returned, communication probably failed
	if ((v == 0x00) || (v == 0xFF)) {
		ESP_LOGI(LOG_TAG, "WARNING: Communication failure, is the MFRC522 properly connected?");
	}
